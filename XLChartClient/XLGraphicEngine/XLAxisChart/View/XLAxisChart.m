...
-(CGSize)textUnit{
    return [@"Unit:" sizeWithAttributes:self.fontAttr];
}
...
-(void)drawTopAxisData{
    ...
    for (index = 0; index < self.xTopAxisScaleDatas.count; index++) {
        ...
        if (maxAxisScaleValue == scaleValue) if(divisor == kDivisor)[_pGraphicEngine mDrawText:@"Unit:" Point:CGPointMake(x - self.textUnit.width/2, y) textAttr:self.fontAttr];
    }
...
-(void)drawLeftAxisData{
    ...
    if(divisor == kDivisor)[_pGraphicEngine mDrawText:@"Unit:" Point:CGPointMake(0, self.topAxisMargin + self.yLeftAxisHeight + self.text10000.height/2) textAttr:self.fontAttr];
